//
//  NormalMappingRenderer.swift
//  OpenGLScratch
//
//  Created by Jagyum Koo on 01/08/2017.
...
class NormalMappingRenderer: MyOpenGLRendererDelegate {
    ...
    var normalMappingProgram: MyOpenGLProgram?
    ...
    func prepareProgram() {
        self.normalMappingProgram = MyOpenGLUtils.createProgramWithName(name: "NormalMapping")
    }
    ...
    func prepareTexture() {
        self.diffuseMap = MyOpenGLTexture(imageName: "brickwall")
        self.normalMap = MyOpenGLTexture(imageName: "brickwall_normal")
    }
    ...
    func render(_ bounds: NSRect) {
        ...
        self.normalMappingProgram?.useProgramWith {
            (program) in
            ...
            let lightModel = GLKMatrix4Scale(GLKMatrix4MakeTranslation(lightPos.x, lightPos.y, lightPos.z), 0.1, 0.1, 0.1)
            program.setMat4(name: "model", value: lightModel)
            self.diffuseMap?.useTextureWith(target: GL_TEXTURE0) {
                self.normalMap?.useTextureWith(target: GL_TEXTURE1) {
                    self.quadVertexObject?.useVertexObjectWith {
                        (vertexObject) in
